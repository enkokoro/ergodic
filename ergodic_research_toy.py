...
opti.minimize(e_plan + sparsemetric)
...
ax4.set_title('Planned Trajectory')
...
pos_plan_data = [([], []) for i in range(N)]
time_data = []
ergodicity_data = []
...
for i in range(N):
    ...
    pos_plan_ln, = ax4.plot(pos_plan_data[i][0], pos_plan_data[i][1], c=colors[i], label=i)
    pos_plan_lns.append(pos_plan_ln)
...
def animate2d_init():
    ...
    return (consensus_ln, sparsity_ln, erg_vs_sparse_ln, ergodicity_ln, planned_ergodicity_ln, *local_erg_lns, *pos_lns, *pos_plan_lns)
...
def animate2d_from_logs_update(frame):
    ...
    for i in range(N):
        ...
        pos_plan_data[i][0].append(x_plan[i][frame][0])
        pos_plan_data[i][1].append(x_plan[i][frame][1])
        pos_plan_lns[i].set_data(pos_plan_data[i][0], pos_plan_data[i][1])
